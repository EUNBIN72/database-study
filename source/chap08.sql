...
SELECT 그룹화_컬럼, 집계_함수(일반_컬럼) FROM 테이블명 WHERE 조건 GROUP BY 그룹화_컬럼;
SELECT gender, AVG(height) FROM students GROUP BY gender;  -- 성별을 기준으로 그룹화
...
SELECT city AS 도시, YEAR(sale_date) AS '판매 연도', SUM(amount) AS '총 매출'  -- 3) 특정 도시의 연도별 총 매출 집계 조회
...
-- 잘못된 컬럼 사용 예시
SELECT id, gender, AVG(height) FROM students GROUP BY gender;  -- id 컬럼 사용 불가

-- 이렇게는 가능
SELECT SUM(id), gender, AVG(height) FROM students GROUP BY gender; 
...
GROUP BY 그룹화_컬럼 
HAVING 그룹_필터링 조건;  -- 그룹화된 결과에 대해 필터링하는 그룹 단위 필터링

-- 결제 유형별 평규 결제 금액은?
SELECT ptype, AVG(amount) FROM payments GROUP BY ptype;

-- 결제 유형별 평균 결제 금액이 40,000원 이상인 데이터는?
SELECT ptype, AVG(amount) FROM payments GROUP BY ptype HAVING AVG(amount) >= 40000;

-- ** WHERE 절은 GROUP BY나 집계 함수가 처리되기 전에 실행됨
-- *** WHERE 절과 HAVING 절을 잘 구분해야 됨
...
SELECT * FROM 테이블명 WHERE 조건 ORDER BY 정렬_컬럼1 [ASC | DESC], 정렬_컬럼2 [ASC | DESC], ...;
...
-- 결제 금액에 대한 정렬: 내림차순
SELECT ptype AS '결제 유형', amount AS '결제 금액'  -- 2) 필요한 컬럼만 선택하고, 별칭을 부여하고
...
FROM payments ORDER BY ptype ASC, amount DESC; -- ptype의 오름차순 정렬 후, amount의 내림차순 정렬(순서 중요)

-- 3. 조회 개수 제한(LIMIT, OFFSET)
-- 조회 결과 중 상위 N개의 레코드만을 조회하는 명령
-- LIMIT 절을 이용해 반환하려는 레코드의 개수를 정의

SELECT 컬럼1, 컬럼2, ... FROM 테이블명 LIMIT N;
...
SELECT 컬럼1, 컬럼2, ... FROM 테이블명 LIMIT N OFFSET M;  -- N: 가져올 레코드의 개수, M: 건너뛸 레코드의 개수
-- 또는 LIMIT M, N; 

-- 결제 금액 4등 ~ 6등까지 조회
SELECT *  -- 2) 모든 컬럼을 선택
FROM payments  -- 1) payment 테이블에서
...
SELECT nickname AS '주문자명', SUM(amount) AS '주문 총액'  -- 6) 특정 컬럼을 조회
FROM users  -- 1) 사용자 테이블에
JOIN orders ON users.id = orders.user_id  -- 2) 주문 테이블을 붙이고
JOIN payments ON orders.id = payments.order_id  -- 3) 결제 테이블도 붙이고
GROUP BY nickname  -- 4) 사용자별 그룹화를 진행하여
HAVING SUM(amount) >= 30000;  -- 5) 그룹 필터링을 하고


-- 가장 많이 팔린 상품 TOP3
-- 상품별 판매 수량을 집계했을 때, 가장 많이 팔린 상품 TOP3 상품명과 판매수량은?
SELECT name AS '상품명', SUM(count) AS '판매 수량'  -- 5) 특정 컬럼 조회
FROM orders   --1) 주문 테이블에
JOIN order_details ON orders.id = order_details.order_id -- 2) 테이블들을 조인하고
JOIN products ON order_details.product_id = products.id 
WHERE status = '배송 완료'  -- 3) 조건 실행
GROUP BY name  -- 4) 그룹화 진행
ORDER BY SUM(count) DESC, name ASC  -- 6) 순서 정렬
LIMIT 3; -- 7) 가져올 개수
...
-- 상품 가격 기준 이론상 매출액: 실제 경제와 무관하며, 할인/부분결제/결제 누락 등을 반영하지 않음
