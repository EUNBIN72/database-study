...
SELECT * FROM students WHERE score > (평균_점수);  -- () 괄호 안이 서브쿼리로 작성할 부분
...
-- 다중 값 서브쿼리: 여러 레코드를 반환하는 서브쿼리(N행 M열) - IN, ANY, ALL, EXIST 연산자와 함께 사용됨
...
-- ① 서브쿼리는 메인쿼리 내부에 중첩해 작성한다. ( O )
-- ② 서브쿼리는 다양한 위치에서 사용할 수 있다. ( O )
-- ③ 서브쿼리는 단일 값만 반환한다. ( X )
...
SELECT payment_type AS '결제 유형', amount AS '결제 금액', amount-(평균결제금액) AS '평균 결제 금액과의 차이'
FROM payments;
...
-- 잘못된 사용 예
-- SELECT 절에 두개 이상의 값을 내면 어떤 결과를 반환하는지 모르기 때문
SELECT 
  payment_type AS '결제 유형', 
  amount AS '결제 금액', 
...
SELECT order_id, SUM(count) AS total_count FROM order_details GROUP BY order_id;
-- 위 값을 서브쿼리로 사용
...
FROM (
  -- 서브쿼리
  SELECT order_id, SUM(count) AS total_count  -- 집계 함수 결과에 별칭 필수(컬럼명이 아니라 계산된 값을 반환하기 때문에)
...
SELECT 
  name AS 상품명,
  total_count AS '주문 개수'
FROM products JOIN (
  -- 서브쿼리: 상품 아이디별 주문 개수 집계
  SELECT product_id, SUM(count) AS total_count
  FROM order_details
  GROUP BY product_id
) AS sub ON sub.product_id = products.id;


-- (참고) 다른 방법: 일단 조인으로 붙여놓고 그룹화 및 집계 - 순서 차이
SELECT 
  name AS 상품명,
  SUM(count) AS '주문 개수'
FROM order_details 
...
SELECT *
FROM products WHERE price > (평균가격);
...
FROM order_details
WHERE product_id IN (
  -- 서브쿼리: '우유 식빵', '크림 치즈'의 아이디를 반환(Nx1)
  SELECT id 
  FROM products
...
-- '우유 식빵', '크림 치즈'를 주문한 사용자의 아이디와 닉네임
...
-- 4. EXISTS 연산자
-- 입력받은 서브쿼리가 하나 이상의 행을 반환하는 경우 TRUE, 그렇지 않으면 FLASE를 반환
...
SELECT * FROM users
WHERE EXISTS (
  -- 서브쿼리: 주무자 아이디가 사용자 테이블에 있다면 1 반환
...
-- orders 테이블을 반복하며 확인, 있으면 결과 테이블에 1 반환
-- 같은 방식으로 2번 사용자, 2번 사용자도 확인
...
WHERE NOT EXISTS (
  ...
  WHERE orders.user_id = users.id AND payment_type = 'COCOA PAY'
);
-- 의미: 주문과 결제 테이블에서 'COCOA PAY'를 사용한 사용자가 있다면 그 놈을 제외한 사용자를 뽑아줘
