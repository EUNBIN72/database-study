...
CREATE TABLE burgers (
    ...
    -- VARCHAR(N): 문자를 최대 N자리까지 저장, 문법상 필수(가변 길이 타입이라 저장될 수 있는 최대 길이 제한을 알아야 하기 때문)
...
-- 데이터 조회하기
-- SELECT 컬럼명1, 컬럼명2, ...  -- 컬럼명 대신 *을 사용하면 모든 컬럼 조회
-- FROM 테이블명
-- WHERE 조건;  -- 검색 조건(생략하면 전체 조회)

-- burgers 테이블의 모든 컬럼 조회
SELECT * FROM burgers;  -- 버거 테이블에서 모든 컬럼을 조회
...
SELECT name, price FROM burgers; -- 버거 테이블에서 특정 컬럼을 조회(이름, 가격)
...
-- -- 어떤 컬럼에 어떤 값을 입력할지 수정 대상을 찾는 조건(생략하면 전체 대상)
...
-- 특정 대상값 변경 시, 조건은 반드시 기본키를 사용
-- 왜? 안전모드에서는 KEY(기본키나 유니크 키) 컬럼이 나니 다른 값을 사용하면 막음
...
-- 데이터 삭제하기: DELETE
-- DELETE FROM 테이블명 WHERE 조건; -- 삭제 대상을 찾는 조건

-- '슈비두밥 버거'가 단종됐다면, 이를 위한 데이터 삭제 쿼리는?
DELETE FROM burgers WHERE name='슈비두밥 버거';
-- 에러 발생: 안전모드로 인해 수정과 동일하게 조건은 반드시 기본키를 사용
