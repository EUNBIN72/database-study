-- Active: 1749606788911@@127.0.0.1@3306@stargram
/*
7.  테이블 조인하기
7.1 조인이란
*/
...
-- 조인의 특징 7가지
-- 1) 조인 컬럼이 필요
-- 2) 두 테이블을 연결하기 위한 공통 컬럼이 필요
-- 보통 외래키와 기본키를 기준으로 조인인
SELECT * FROM comments JOIN users ON comments.user_id = users.id;  -- 조인 컬럼(외리키) = 조인 컬럼(기본키)

-- 2) 조인 컬럼은 서로 자료형이 일치해야 조인 가능
...
-- 4. 연속적인 조인 가능
--  연속 조인 연습
SELECT FROM    -- 4(원하는 컬럼 조회)
FROM comments  -- 1) 댓글 테이블에 
JOIN users ON commemts.user_id = users.id  -- 2)사용자 테이블 조인하고
JOIN photos On comments.photo_id;        -- 3) 다시 사진 테이블 조인 후

-- 5) 중복 컬럼은 테이블명을 붙여 구분
-- 컬럼명이 같은 경우 어느 테이블의 것인지 명시해야 함(그렇지 않은 경우 에러 발생)
-- 사용 예: 중복 컬럼 id에 테이블명 명시
SELECT comments.id, body, user_id, nickname
FROM comments
...
-- photos 테이블과 users 테이블 INNER JOIN
USE mapdonalds;
SELECT * FROM photos AS p JOIN users AS u ON p.user_id = u.id;
...
-- photos 테이블과 users 테이블 LEFT JOIN
...
-- photos 테이블과 users 테이블 RIGHT JOIN
SELECT * FROM photos AS p
...
SELECT COUNT(*)
FROM users AS u  -- 1) 사용자 정보를 가지고
JOIN photos AS p  -- 2) 사진 정보를 합쳐서
    ON u.id = p.user_id 
JOIN likes AS l  -- 3) 좋아요 정보도 합쳐서
    ON p.id = l.photo_id
WHERE nickname = '홍팍';  -- 홍팍이 올린린
...
-- ② INNER 조인은 조인이 불가능한 튜플도 가져와 연결한다. ( X )
