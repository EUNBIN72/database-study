...
-- 부동 소수점 vs 고정 소수점
-- 부동 소수점: 넓은 범위 표현 가능, 숫자 계산에 오차 발생(2진수 기반이라서)
-- FLOAT DOUBLE 타입은 0.1을 정확히 저장하지 않고 근사값으로 저장(왜? 0.1은 2진수로 무한 소수)
-- FLOAT: 약 7자리 정확도
-- DOUBLE: 약 15~17자리 정확도
-- 고정 소수점: 특정 범위 안에서 정확한 연산을 수행(10진수 기반이라서)
-- DECIMAL 타입은 0.1을 근사값이 아니라 정확하게 저장
-- 정확히 소수값이 필요하면 DECIMAL을 사용
...
CREATE TABLE student_records (
  id INTEGER PRIMARY KEY,       -- 아이디(표준 점수)
...
CREATE TABLE addresses (
  potal_code CHAR(5), -- 우편번호(고정 길이 문자 : 5자), 주로 고정된 길이의 코드값(예: 국가코드드)
  -- 문자를 3개만 넣는 경우, 자동으로 공백(spcae) 문자를 채움(예: 'abc  )
  streeet_address VARCHAR(100)-- 거리 주소(가변 길이 문자: 최대 100자)
...
CREATE Table user_profiles (
  id INTEGER PRIMARY KEY,      -- 아이디(표준 점수), 기본키 지정: id
...
-- 이벤트(events) 테이블 생성
CREATE TABLE events (
  id INTEGER PRIMARY KEY, 	-- 아이디(표준 정수), 기본키 지정: id
...
-- LIKE 연산자
-- 컬럼에서 특정 패턴과 일치하는 대상(문자열)을 찾아 필터링을 수행
...
-- 11월에 주문받은 상품 조회
SELECT * FROM orders WHERE MONTH(created_at) = 11;

-- 11월에 주문받은 상품의 개수 총합
SELECT SUM(quantity) FROM orders WHERE MONTH(created_at) = 11;
...
-- 오전에 주문받은 모든 상품 조회
SELECT * FROM orders WHERE HOUR(created_at) < 12;

-- 오전에 주문받은 모든 상품 매출의 합계 조회
SELECT SUM(price * quantity) FROM orders WHERE HOUR(created_at);
...
SELECT COUNT(*) FROM orders WHERE created_at BETWEEN '2024-11-15' AND '2024-12-15';
