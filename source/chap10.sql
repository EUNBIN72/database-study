-- Active: 1749606788911@@127.0.0.1@3306@data_modeling
/*
10. 데이터 모델링
10.1 데이터 모델링이란
*/
...
-- ① 데이터 모델링이란 현실 세계의 사물이나 현상을 추상화해 데이터 간 구조와 관계를 정의한 것이다. ( O )
-- ② 데이터 모델링할 때는 비즈니스 요구사항은 무시하고 데이터 구조만 고려한다. ( X )
-- ③ 데이터 모델링하면 추후 시스템 확장과 변화에 쉽게 적응할 수 있다. ( O )
-- ④ 개념적 데이터 모델링 단계에서는 요구사항 분석을 통해 핵심 엔티티와 이들 간 관계를 정의한다. ( O )
-- ⑤ 논리적 데이터 모델링 단계에서는 개념적 데이터 모델링의 결과 만들어진 데이터 모델의 세부 내용을 정의한다. ( O )
-- ⑥ 물리적 데이터 모델링 단계에서는 논리적 데이터 모델링의 결과 만들어진 데이터 모델을 특정 DBMS에 맞게 최적화한다.( O )
...
-- ③ 학생 엔티티와 강의 엔티티는 필수적 다대다 관계이다.  --> 선택적적
...
CREATE Table payments(
  ...
  order_id INTEGER NOT NULL UNIQUE,  -- 주문 아이디(필수 입력, 고유키 지정)  <-- 필수적 일대일 관계                                                                                                                                                                                                                                                                            ,  -- 주문 아이디(필수 입력, 고유키 지정)
  PRIMARY KEY(id),  -- 기본키 지정
  FOREIGN KEY (order_id) REFERENCES orders(id)  -- 외래키 지정: user_id 
);
