...
-- 종류: AND, OR, NOT
-- 사용 예
-- 조건A AND 조건B: 조건A와 조건B를 동시에 만족하는 데이터를 필터링(교집합)
-- 조건A OR 조건B: 조건A와 조건B중 하나라도 만족하는 데이터를 필터링(합집합)
-- NOT 조건A: 조건A를 만족하지 않는 데이터를 필터링(여집합)
...
-- 산술 연산자 활용 예
-- SELECT문에서의 산술 연산 예시
SELECT *, price / gram * 100 AS 'price/100g' FROM burgers;
-- 별칭에서의 특수기호는 따옴표("",'') 또는 백틱(``)으로 감싸기
-- (작은따옴표를 주로 씀)
...
-- 3. 모든 학생의 총점은?
SELECT *,(math + english + programming) AS totalscore FROM students;


-- 4. 모든 학생의 평균은?
SELECT *,(math + english + programming)/3 AS avg FROM students;


-- 5. 총점이 270 이상인 학생의 닉네임, 총점, 평균은?
SELECT nickname, (math + english + programming) AS totalscore, (math + english + programming)/3 AS avg
FROM students WHERE (math + english + programming) >= 270 ;
