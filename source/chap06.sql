...
-- 예: 주민등록번호
-- 다음 4가지 특징을 만족해야 함
-- - 유일성: 유일한 값을 가져야 함(=UNIQUE 제약 조건) 중복이 안되는 유일한 조건
...
CREATE TABLE users (
  ...
  age INTEGER CHECK (age >= 18),         -- 나이(18세 이상만 가입 허용)
...
CREATE TABLE capitals (
  ...
  -- 타입은 국가 테이블과 동일하게 맞춤
  -- 고유한 값만 허용(한 나라에 수도가 2개가 있으면 안됨) => UNIQUE : 일대일 관계 만들기 위한 중요한 제약 조건
...
CREATE TABLE passports (
  ...
  FOREIGN (person_id) REFERENCES persons(id)
);
...
--     => '다' 쪽에 외래키 저장 
-- ③ 두 테이블 간 다대다 관계는 중간 테이블을 만들어 두 테이블의 기본키를 참조하도록 외래키를 지정한다. ( O )
...
-- 1. 사용자 테이블 만들기
CREATE TABLE users (
  id INTEGER AUTO_INCREMENT PRIMARY KEY,  -- 아이디(자동으로 1씩 증가), 기본키 지정
...
CREATE TABLE comments (
  ...
  photo_id INTEGER,                       -- 댓글에 달린 사진 아이디
  FOREIGN KEY (user_id) REFERENCES users(id),  -- 외래키 지정: user_id
