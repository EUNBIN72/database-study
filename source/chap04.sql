...
-- 최대값, 최소값
-- MAX(), MIN()
SELECT MAX(price), MIN(price) FROM burgers;
...
-- 레코드(튜플)의 개수 세기
SELECT * FROM burgers WHERE gram > 240;
SELECT COUNT(*) FROM burgers WHERE gram > 240;

-- 주의
-- COUNT() 함수는 입력에 따라 다른 결과를 만듦
-- COUNT(*) : 조회된 전체 레코드 수를 반환
...
CREATE TABLE transactions (
  id INTEGER PRIMARY KEY,  -- 아이디 / 기본키 지정
...
-- 쿠팡 및 아마존과 거래한 총 횟수는?
SELECT COUNT(*) FROM transactions WHERE msg='Coupang' OR msg='Amazon';

-- 위 쿼리를 IN 연산자를 활용한 버전으로 다시 작성한다면?
SELECT COUNT(*) FROM transactions WHERE msg IN ('Coupang', 'Amazon');
...
SELECT AVG(amount) FROM transactions WHERE msg IN ('Google', 'Amazon') AND amount > 0;

-- 거래처 목록 조회하기
-- 거래처를 담은 msg만 조회하면? 중복된 결과가 나옴옴
