...
-- Quiz
-- 1. 다음 빈칸에 들어갈 용어는?
-- ____DB____(이)란 구조화된 데이터의 집합으로, 일종의 데이터 창고와 같습니다.

-- 2. 다음 빈칸에 들어갈 용어는?
-- ____DBMS____(이)란 데이터베이스를 관리하기 위한 프로그램으로 데이터베이스에 저장된 데이터를 생성, 조회, 수정, 삭제하는 등 다양한 방식으로 데이터를 조작할 때 활용합니다.

-- 3. 다음 빈칸에 들어갈 용어는?
-- ___SQL____(이)란 사용자와 데이터베이스가 소통하기 위한 언어로 데이터베이스에 저장된 데이터를 가져오거나 변경하기 위한 명령을 내릴 때 사용합니다.
...
--  기능적으로는 소문자, 대문자 차이 없음, 전적으로 가독성과 팀의 코딩 스타일 가이드에 따라 달라짐
--  관례적으로는 SQL 키워드는 대문자로 씀

-- Quiz
-- 5. 다음 빈칸에 들어갈 용어는?
-- ___서버/server___(이)란 특정 서비스를 제공하는 프로그램(또는 컴퓨터)을 뜻합니다.
-- 여기서는 MySQL 서버가 이에 해당합니다.

-- 6. 다음 빈칸에 들어갈 용어는?
-- ___클라이언트/client___(이)란 서버가 제공하는 기능을 사용하는 프로그램(또는 컴퓨터)을 뜻합니다.
