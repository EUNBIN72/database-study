...
-- 정답: 1 (속성 정의는 개념적 모델링에서도 하고, 논리적 모델링에서도 함함)
...
-- 정답: 2
-- 한 명의 회원은 여러 대출 기록을 가질 수 있지만, 각 대출 기록은 특정 회원에만 속하므로 일대다 관계가 성립립


-- 문제 3
-- 도서관 데이터베이스의 물리적 모델링 단계에서 수행해야 하는 작업은 무엇입니까?

-- ① 대출 기록 테이블에서 borrow_date 칼럼에 기본값으로 현재 날짜를 설정한다.
-- ② 회원과 대출 기록 간의 관계를 일대다로 정의한다.  ---> 논리적
-- ③ 회원 테이블과 대출 기록 테이블 간의 ER 다이어그램을 작성한다.  --> 논리적
-- ④ 대출 기록 테이블에서 borrow_date 칼럼을 생성한다.  --> 논리적
...
-- 정답: 2
-- 각 속성은 원자값(더 이상 분해할 수 없는 값)을 가지고, 같은 속성끼리 동일한 자료형을 가지며,
-- 각 인스턴스는 고유하므로 제1정규형을 만족
-- 기본키 후보는 {member_id, book_id}의 복합키이며,
-- 이때 member_name은 member_id에만 종속되고,
-- book_title과 categoty는 book_id에만 종속된다.
-- 따라서 이 경우 부분 함수 종속이 존재하므로, 제2정규형 위반
