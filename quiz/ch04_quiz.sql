...
-- 정답:
SELECT COUNT(*) FROM orders WHERE price >= 1000000;
...
-- 정답:
SELECT SUM(quantity * price) FROM orders WHERE quantity >= 2 AND price <= 1000000;
...
-- 정답:
SELECT SUM(quantity) FROM orders WHERE order_date >= '2023-11-01' AND order_date <= '2023-11-05';
-- DATE  타입이라 날짜로 변환되어 비교를 함
-- 문자열 비교여도 문제는 없음
...
-- 정답:
SELECT DISTINCT product FROM orders WHERE customer_name = '김철수' OR region = '대구';
...
-- 정답:
SELECT SUM(quantity) FROM orders WHERE product='스마트폰';
