...
-- 정답: 4
-- 중복 레코드를 제거하고 합치는 UNION 연산자를 사용했을 때와 결과가 같음
...
-- 정답: 
SELECT name, date_of_birth, address FROM members JOIN member_profiles ON members.id = member_profiles.member_id;
...
-- 정답:
SELECT name, date_of_birth, address FROM members LEFT JOIN member_profiles ON members.id = member_profiles.member_id;
...
-- 정답:
SELECT name, title FROM  borrow_records
...
-- 정답:
SELECT title, borrow_date FROM books LEFT JOIN borrow_records ON books.id = borrow_records.book_id;
...
-- 일반적으로는 관계 있는 테이블끼리 JOIN을 수행행
SELECT members.name, role FROM members JOIN library_staff ON library_staff.name = members.name;
