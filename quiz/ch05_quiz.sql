...
-- 정답: 메모리 -- 정답: SMALLINT는 2바이트로 메모리를 절약하면서 65,535까지의 정수를 저장할 수 있기 때문이다.
...
-- 정답:
SELECT title, attendees FROM events WHERE attendees BETWEEN 50 AND 150;
...
-- 정답:
SELECT title, event_date FROM events WHERE DATE(event_date) BETWEEN '2023-10-01' AND '2023-12-31';
...
-- 정답:
SELECT title, start_time FROM events WHERE title LIKE '%컨퍼런스%' OR title LIKE '%컨트리뷰션%' AND
	HOUR(start_time) >= 11;
...
WHERE DATE(event_date) BETWEEN '2023-11-1' AND '2023-12-31' AND
HOUR(start_time) >= 14;
